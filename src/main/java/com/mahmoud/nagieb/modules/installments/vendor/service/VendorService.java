...
import com.mahmoud.nagieb.exception.*;
import com.mahmoud.nagieb.modules.installments.purchase.entity.Purchase;
import com.mahmoud.nagieb.modules.installments.vendor.dto.PurchaseDTO;
...
import com.mahmoud.nagieb.modules.installments.vendor.mapper.VendorMapper;
...
public class VendorService {
    ...
    public  VendorResponse getById(Long id) {
        Vendor vendor = vendorRepository.findWithPurchase(id)
            ...
            List<PurchaseDTO> purchases = vendor.getPurchases().stream()
                    .map(p -> new PurchaseDTO(p.getProductName(), p.getBuyPrice()))
                    .toList();
    ...
    public Page<VendorSummary> list(int page, int size, String search) {
        ...
        if (search == null || search.isBlank()) {
            ...
            return vendorRepository.findByNameContainingIgnoreCaseAndActiveTrue(search, pageable)
                    .map(VendorMapper::toVendorSummary);
        }
    }
