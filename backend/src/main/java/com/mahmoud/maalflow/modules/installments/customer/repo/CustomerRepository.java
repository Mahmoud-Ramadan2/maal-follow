...
//    Page<Customer> searchCustomers(@Param("search") String search, Pageable pageable);
    ...
    @Query("""
            SELECT c FROM Customer c WHERE c.active = :status AND
            (:searchTerm IS NULL OR LOWER(c.name) LIKE LOWER(CONCAT('%', :searchTerm, '%')) OR
            LOWER(c.phone) LIKE LOWER(CONCAT('%', :searchTerm, '%')) OR
            LOWER(c.nationalId) LIKE LOWER(CONCAT('%', :searchTerm, '%')) OR
            LOWER(c.address) LIKE LOWER(CONCAT('%', :searchTerm, '%')) OR
            LOWER(c.notes) LIKE LOWER(CONCAT('%', :searchTerm, '%')))
""")
    Page<Customer> searchCustomers(Pageable pageable, @Param("searchTerm") String searchTerm,  @Param("status") boolean status);

    // retrun all customers regardless of active status but filter by search term
    @Query("""
            ...
            LOWER(c.notes) LIKE LOWER(CONCAT('%', :searchTerm, '%')))
""")
    Page<Customer> searchCustomers(Pageable pageable, @Param("searchTerm") String searchTerm);
    ...
    """)
    long countActiveCustomersHasActiveContracts();
}
