...
public interface VendorRepository extends JpaRepository<Vendor, Long> {
   ...
   Page<Vendor> findByNameContainingIgnoreCaseAndActiveTrue(String search, Pageable pageable);
    ...
    Page<Vendor> findByNameContainingIgnoreCase(String search, Pageable pageable);

    Page<Vendor> findByNameContainingIgnoreCaseAndActiveFalse(String search, Pageable pageable);

    @Query("SELECT v FROM Vendor v LEFT JOIN FETCH v.purchases WHERE v.id = :id ")
...
""")
    ...
    @Query("""
            SELECT v FROM Vendor v WHERE
                    (:status IS NULL OR v.active = :status) AND
                    ...
                    LOWER(v.notes) LIKE LOWER(CONCAT('%', :searchTerm, '%'))
                     )
""")
    Page<Vendor> searchVendor(Pageable pageable,@Param("searchTerm") String searchTerm, @Param("status") Boolean status);
}