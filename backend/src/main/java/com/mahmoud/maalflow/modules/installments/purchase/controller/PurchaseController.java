...
//        return ResponseEntity.status(200).body(productService.list(page, size, sort, search));
//    }

    @GetMapping
    public ResponseEntity<Page<PurchaseResponse>> listPurchases(Pageable pageable, PurchaseFilter filter) {
        return ResponseEntity.status(200).body(productService.list(pageable, filter));
    }

    @GetMapping("/statistics")
    public ResponseEntity<PurchaseStatistics> getPurchaseStatistics() {
