package com.mahmoud.maalflow.modules.installments.customer.controller;
...
import com.mahmoud.maalflow.modules.installments.customer.service.CustomerService;
...
public class CustomerController {
    ...
    @GetMapping
    public ResponseEntity<Page<CustomerSummary>> list(Pageable pageable,
                                                      @RequestParam(required = false) String search,
                                                      @RequestParam(defaultValue = "active", required = false) String status) {

        Page<CustomerSummary> customerPage  = customerService.list(pageable, search, status);
    ...
    public ResponseEntity<Map<String, Long>> getCustomerCount() {
        ...
        long countActiveWithContracts = customerService.getActiveCustomerCountHasActiveContracts();

        String activeMessage = messageSource.getMessage("customer.count.active", null, LocaleContextHolder.getLocale());
        String inactiveMessage = messageSource.getMessage("customer.count.inactive", null, LocaleContextHolder.getLocale());
        String activeWithContractsMessage = messageSource.getMessage("customer.count.active.has.contracts", null, LocaleContextHolder.getLocale());

        return ResponseEntity.ok(Map.of(
                activeWithContractsMessage, countActiveWithContracts,
